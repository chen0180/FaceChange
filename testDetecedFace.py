...
OUTPUT_PATH='H:/cmq/securityAI_round1_images/outputs/'
...
class FaceChanger(object):
    ...
    def __init__(self, which_predictor='68'):
        ...
        self.ALIGN_POINTS = (self.LEFT_BROW_POINTS + self.RIGHT_EYE_POINTS + self.LEFT_EYE_POINTS +
                             self.RIGHT_BROW_POINTS + self.NOSE_POINTS + self.MOUTH_POINTS+self.JAW_POINTS+self.FACE_POINTS)

        self.OVERLAY_POINTS = [
            self.LEFT_EYE_POINTS + self.RIGHT_EYE_POINTS + self.LEFT_BROW_POINTS + self.RIGHT_BROW_POINTS
            +self.NOSE_POINTS+self.MOUTH_POINTS+self.JAW_POINTS+self.FACE_POINTS
        ]
        #
        # self.OVERLAY_POINTS = [
...
def get_128vectorfromImage(img_bgr):
    """
    根据图片路径得到128维向量
    :param img_path:
    :return: li :list
    ...
    print('get vector from image_bgr need time:{}'.format(vec_end_time-vec_start_time))
    return li
...
def get_maxdistance_index_from_distanceall(distance_all):
    """
    从distance中找到第二小的index,dist csv文件中读取
    :param distance_all:(694,694)
    :return:(694)
    """
    pic_num=len(distance_all) #694
    print("pic num:{}".format(pic_num))

    max_index_all=[]

    for i in range(len(distance_all)):
        dist1=distance_all[i] #第i幅图片 与所有图片的距离
        # print('pic :{},distance:{}'.format(i+1,dist1))
        max_dist = 0.0
        max_index = i
    ...
    print('all max index:len={}, {}'.format(len(max_index_all),max_index_all))
    return max_index_all
...
def get_distance_all(imageName):
    ...
    for i in range(pic_number):
        ...
        for j in range(pic_number):
            ...
            print('dist of pic {},pic {} is {}'.format(i + 1, j + 1, dist))

            dist_pic.append(dist)
        dist_all.append(dist_pic)

        pic_end_time = time.time()
        print('get pic={} 496 distance need time={}'.format(i+1,pic_end_time-pic_start_time))
...
if __name__ == '__main__':
    IMAGES_DEV_1 = 'H:/cmq/securityAI_round1_dev_new.csv' #完整的
    imageName=get_image_names(IMAGES_DEV_1)
    print(imageName.shape)
    # imageName_1=imageName[0:174,:]
    # print(imageName_1.shape)
    # print(imageName_1[0])
    # print(imageName_1[-1])

    for i in range(len(imageName)):
        img_path=IMAGES_PATH+imageName[i]
        img_path=img_path[0]
        print(img_path)
        vector=get_128vector(img_path)
        # print(vector)
